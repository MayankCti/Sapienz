...
function ExpertDashboard() {
  ...
  const titleToRouteMap = {
    [cardData[0]?.title]: pageRoutes?.student,
    [cardData[1]?.title]: pageRoutes?.experts,
    [cardData[2]?.title]: pageRoutes?.flashCard,
    [cardData[3]?.title]: pageRoutes?.categories,
  };
  ...
  const navigateTo = (title) => {
    console.log(title);
    const route = titleToRouteMap[title];
    if (route) {
      navigate(route);
    }
  };
